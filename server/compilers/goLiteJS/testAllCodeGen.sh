...
VALID_DIR=./src/test/ml4/valid/*.go
...
do
  ...
  ./testcodegen.sh $PROG 2>&1 | tee -a valid.log | tr -d '\n'
...
INVALID_DIR=./src/test/ml4/invalid/*.go
...
do
  ...
  ./testcodegen.sh $PROG 2>&1 | tee -a invalid.log | tr -d '\n'
